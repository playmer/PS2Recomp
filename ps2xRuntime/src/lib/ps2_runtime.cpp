...
static constexpr uint32_t DEFAULT_FB_ADDR =
    (PS2_RAM_SIZE - DEFAULT_FB_SIZE - 0x10000u);
struct ElfHeader {
  uint32_t magic;
  uint8_t elf_class;
  uint8_t endianness;
...
void raiseCop0Exception(R5900Context *ctx, uint32_t exceptionCode,
                        bool tlbRefill = false) {
  ctx->cop0_epc = ctx->pc;
  ctx->cop0_cause =
      (ctx->cop0_cause & ~(COP0_CAUSE_EXCCODE_MASK | COP0_CAUSE_BD)) |
...
PS2Runtime::IoPaths &runtimeIoPaths() {
  static PS2Runtime::IoPaths paths = []() {
    ...
    defaults.elfDirectory =
        ec ? std::filesystem::path(".") : cwd.lexically_normal();
...
std::string readGuestPrintableString(const uint8_t *rdram, uint32_t addr,
                                     size_t maxLen) {
  std::string out;
  if (!rdram || maxLen == 0) {
    return out;
  ...
  for (size_t i = 0; i < maxLen; ++i) {
    const char ch = static_cast<char>(
        rdram[(addr + static_cast<uint32_t>(i)) & PS2_RAM_MASK]);
    if (ch == '\0') {
      break;
    }
    if (ch >= 0x20 && ch < 0x7F) {
...
bool PS2Runtime::loadELF(const std::string &elfPath) {
  ...
  const uint32_t paddedEnd =
      (maxLoadedRdramEnd > (PS2_RAM_SIZE - kGuestHeapSafetyPad))
          ? PS2_RAM_SIZE
          : (maxLoadedRdramEnd + kGuestHeapSafetyPad);
  const uint32_t suggestedHeapBase =
      alignGuestHeapValue(paddedEnd, kGuestHeapDefaultAlignment);
  {
...
const PS2Runtime::IoPaths &PS2Runtime::getIoPaths() { return runtimeIoPaths(); }
...
PS2Runtime::RecompiledFunction PS2Runtime::lookupFunction(uint32_t address) {
  ...
  static RecompiledFunction defaultFunction =
      [](uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime) {
        std::cerr << "Error: Called unimplemented function at address 0x"
...
void PS2Runtime::executeVU0Microprogram(uint8_t *rdram, R5900Context *ctx,
                                        uint32_t address) {
  static std::unordered_map<uint32_t, int> seen;
  int &count = seen[address];
  if (count < 3) {
    std::cout << "[VU0] microprogram @0x" << std::hex << address << " pc=0x"
              << ctx->pc << " ra=0x"
...
void PS2Runtime::vu0StartMicroProgram(uint8_t *rdram, R5900Context *ctx,
                                      uint32_t address) {
...
void PS2Runtime::handleSyscall(uint8_t *rdram, R5900Context *ctx,
  ...
  // Try immediate first
  if (encodedSyscallId != 0 && ps2_syscalls::dispatchNumericSyscall(
                                   encodedSyscallId, rdram, ctx, this)) {
    return;
  }
...
void PS2Runtime::handleTLBR(uint8_t *rdram, R5900Context *ctx) {
  ...
  ctx->cop0_entrylo0 = (ctx->cop0_entrylo0 & ~0x03FFFFC2u) |
                       ((pfn & 0x000FFFFFu) << 6) | (valid ? 0x2u : 0u);
...
void PS2Runtime::resetGuestHeapLocked(uint32_t guestBase, uint32_t guestLimit) {
  uint32_t base = alignGuestHeapValue(clampGuestHeapBase(guestBase),
                                      kGuestHeapDefaultAlignment);
  uint32_t limit = clampGuestHeapLimit(guestLimit);
  if (base == 0u) {
    const uint32_t fallbackBase = (m_guestHeapSuggestedBase != 0u)
                                      ? m_guestHeapSuggestedBase
                                      : kGuestHeapDefaultBase;
    base = alignGuestHeapValue(clampGuestHeapBase(fallbackBase),
                               kGuestHeapDefaultAlignment);
  }

  if (limit <= base) {
    base = alignGuestHeapValue(clampGuestHeapBase(m_guestHeapSuggestedBase),
                               kGuestHeapDefaultAlignment);
...
void PS2Runtime::ensureGuestHeapInitializedLocked() {
  ...
  const uint32_t suggested = (m_guestHeapSuggestedBase == 0u)
                                 ? kGuestHeapDefaultBase
                                 : m_guestHeapSuggestedBase;
...
uint32_t PS2Runtime::allocateGuestBlockLocked(uint32_t size,
                                              uint32_t alignment) {
  if (size == 0u) {
    return 0u;
  }
  ...
  if (size > (std::numeric_limits<uint32_t>::max() -
              (kGuestHeapDefaultAlignment - 1u))) {
    return 0u;
  }

  const uint32_t allocSize =
      alignGuestHeapValue(size, kGuestHeapDefaultAlignment);
  if (allocSize == 0u) {
    return 0u;
  }
  ...
  for (size_t i = 0; i < m_guestHeapBlocks.size(); ++i) {
    ...
    const uint32_t alignedAddr =
        alignGuestHeapValue(block.addr, normalizedAlignment);
    ...
    const uint32_t prefixSize =
        static_cast<uint32_t>(alignedStart - blockStart);
    ...
    if (suffixSize > 0u) {
      replacement.push_back(
          {static_cast<uint32_t>(allocEnd), suffixSize, true});
    }

    m_guestHeapBlocks.erase(m_guestHeapBlocks.begin() +
                            static_cast<std::ptrdiff_t>(i));
    m_guestHeapBlocks.insert(m_guestHeapBlocks.begin() +
                                 static_cast<std::ptrdiff_t>(i),
...
void PS2Runtime::coalesceGuestHeapLocked() {
  ...
  while (i < m_guestHeapBlocks.size()) {
    ...
    const uint64_t prevEnd =
        static_cast<uint64_t>(prev.addr) + static_cast<uint64_t>(prev.size);
...
void PS2Runtime::configureGuestHeap(uint32_t guestBase, uint32_t guestLimit) {
  std::lock_guard<std::mutex> lock(m_guestHeapMutex);
  uint32_t normalizedBase = alignGuestHeapValue(clampGuestHeapBase(guestBase),
                                                kGuestHeapDefaultAlignment);
  if (normalizedBase == 0u) {
    normalizedBase = (m_guestHeapSuggestedBase != 0u) ? m_guestHeapSuggestedBase
                                                      : kGuestHeapDefaultBase;
  }
...
uint32_t PS2Runtime::guestCalloc(uint32_t count, uint32_t size,
                                 uint32_t alignment) {
  if (count == 0u || size == 0u) {
    return 0u;
  }
...
uint32_t PS2Runtime::guestRealloc(uint32_t guestAddr, uint32_t newSize,
                                  uint32_t alignment) {
  if (guestAddr == 0u) {
  ...
  const uint32_t requestedSize =
      alignGuestHeapValue(newSize, kGuestHeapDefaultAlignment);
  ...
  if (requestedSize <= oldSize) {
    if (requestedSize < oldSize) {
      ...
      m_guestHeapBlocks.insert(m_guestHeapBlocks.begin() +
                                   static_cast<std::ptrdiff_t>(blockIndex + 1u),
  ...
  if (blockIndex + 1u < m_guestHeapBlocks.size()) {
    ...
    const uint64_t blockEnd =
        static_cast<uint64_t>(m_guestHeapBlocks[blockIndex].addr) +
    ...
    if (next.free && blockEnd == next.addr) {
      ...
      if (combined >= requestedSize) {
        const uint32_t extraNeeded =
            requestedSize - m_guestHeapBlocks[blockIndex].size;
        m_guestHeapBlocks[blockIndex].size = requestedSize;
  ...
  const uint32_t newAddr =
      allocateGuestBlockLocked(newSize, normalizedAlignment);
  if (newAddr == 0u) {
    return 0u;
  }
...
void PS2Runtime::Store8(uint8_t *rdram, R5900Context *ctx, uint32_t vaddr,
                        uint8_t value) {
...
void PS2Runtime::Store16(uint8_t *rdram, R5900Context *ctx, uint32_t vaddr,
                         uint16_t value) {
...
void PS2Runtime::Store32(uint8_t *rdram, R5900Context *ctx, uint32_t vaddr,
                         uint32_t value) {
...
void PS2Runtime::Store64(uint8_t *rdram, R5900Context *ctx, uint32_t vaddr,
                         uint64_t value) {
...
void PS2Runtime::Store128(uint8_t *rdram, R5900Context *ctx, uint32_t vaddr,
                          __m128i value) {
...
void PS2Runtime::run() {
  ...
  std::thread gameThread([&]() {
    ...
    try {
      ...
      std::cout << "Game thread returned. PC=0x" << std::hex << pc << " RA=0x"
                << static_cast<uint32_t>(
                       _mm_extract_epi32(m_cpuContext.r[31], 0))
                << std::dec << std::endl;
    } catch (const std::exception &e) {
  ...
  while (!gameThreadFinished.load(std::memory_order_acquire)) {
    ...
    if ((tick % 600) == 0) {
      ...
      if (curDma != lastDma || curGif != lastGif || curGs != lastGs ||
          curVif != lastVif) {
        std::cout << "[hw] dma_starts=" << curDma << " gif_copies=" << curGif
                  << " gs_writes=" << curGs << " vif_writes=" << curVif
                  << std::endl;
        lastDma = curDma;
        lastGif = curGif;
        lastGs = curGs;
    ...
    if (WindowShouldClose()) {
      std::cout << "[run] window close requested, breaking out of loop"
                << std::endl;
      requestStop();
      break;
    }
