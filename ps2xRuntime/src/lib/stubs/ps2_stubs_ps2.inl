void sceCdRead(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime) {
  ...
  auto clampReadBytes = [](uint32_t sectors, uint32_t offset) -> size_t {
    const uint64_t requested =
        static_cast<uint64_t>(sectors) * static_cast<uint64_t>(kCdSectorSize);
    if (requested == 0) {
      return 0;
    }
  ...
  if (!ok) {
      ...
      for (const CdReadArgs &candidate : alternatives) {
        ...
        if (tryRead(candidate)) {
          ...
          if (recoverLogCount < 16) {
            std::cout << "[sceCdRead] recovered with alternate args "
                      << candidate.tag << " (pc=0x" << std::hex << ctx->pc
                      << " ra=0x" << getRegU32(ctx, 31) << " a0=0x" << a0
                      << " a1=0x" << a1 << " a2=0x" << a2 << std::dec << ")"
                      << std::endl;
            ++recoverLogCount;
          }
          selected = candidate;
    ...
    if (!ok) {
      ...
      if (unresolvedLogCount < 32) {
        std::cerr << "[sceCdRead] unresolved request pc=0x" << std::hex
                  << ctx->pc << " ra=0x" << getRegU32(ctx, 31) << " a0=0x" << a0
                  << " a1=0x" << a1 << " a2=0x" << a2 << std::dec << std::endl;
        ++unresolvedLogCount;
      }
    }
  }
