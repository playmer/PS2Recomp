...
void syMalloc(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime) {
  ...
  if (runtime && requestedSize != 0u) {
    // Match game expectation for allocator alignment while keeping pointers in
    // EE RAM.
  ...
  if (logCount < 16) {
    std::cout << "ps2_stub syMalloc"
              << " size=0x" << std::hex << requestedSize << " -> 0x"
              << resultAddr << std::dec << std::endl;
    ++logCount;
  }
...
void syMallocInit(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime) {
  ...
  if (runtime) {
    ...
    const bool suspiciousKsegBase = (heapBase & 0xE0000000u) == 0x80000000u &&
                                    normalizedBase < kHeapBaseFloor;
    if (normalizedBase == 0u || suspiciousKsegBase) {
      // Keep the ELF-driven suggestion instead of collapsing heap to low
      // memory.
    ...
    if (heapSize != 0u && heapSize <= PS2_RAM_SIZE &&
        normalizedBase < PS2_RAM_SIZE) {
      const uint64_t candidateLimit = static_cast<uint64_t>(normalizedBase) +
                                      static_cast<uint64_t>(heapSize);
    ...
    if (logCount < 8) {
      std::cout << "ps2_stub syMallocInit"
                << " reqBase=0x" << std::hex << heapBase << " reqSize=0x"
                << heapSize << " normBase=0x" << normalizedBase
                << " reqLimit=0x" << heapLimit << " finalBase=0x"
                << runtime->guestHeapBase() << " finalEnd=0x"
                << runtime->guestHeapEnd() << std::dec << std::endl;
      ++logCount;
    }
  } else if (logCount < 8) {
...
void sndr_trans_func(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime) {
  ...
  // For now just clear the snd busy flag used by sdMultiUnitDownload/SysServer
  // loops.
...
void mcCallMessageTypeSe(uint8_t *rdram, R5900Context *ctx,
                         PS2Runtime *runtime) {
  TODO_NAMED("mcCallMessageTypeSe", rdram, ctx, runtime);
}

void mcCheckReadStartConfigFile(uint8_t *rdram, R5900Context *ctx,
                                PS2Runtime *runtime) {
  TODO_NAMED("mcCheckReadStartConfigFile", rdram, ctx, runtime);
}

void mcCheckReadStartSaveFile(uint8_t *rdram, R5900Context *ctx,
                              PS2Runtime *runtime) {
  TODO_NAMED("mcCheckReadStartSaveFile", rdram, ctx, runtime);
}

void mcCheckWriteStartConfigFile(uint8_t *rdram, R5900Context *ctx,
                                 PS2Runtime *runtime) {
  TODO_NAMED("mcCheckWriteStartConfigFile", rdram, ctx, runtime);
}

void mcCheckWriteStartSaveFile(uint8_t *rdram, R5900Context *ctx,
                               PS2Runtime *runtime) {
  TODO_NAMED("mcCheckWriteStartSaveFile", rdram, ctx, runtime);
}

void mcCreateConfigInit(uint8_t *rdram, R5900Context *ctx,
                        PS2Runtime *runtime) {
  TODO_NAMED("mcCreateConfigInit", rdram, ctx, runtime);
}

void mcCreateFileSelectWindow(uint8_t *rdram, R5900Context *ctx,
                              PS2Runtime *runtime) {
...
void mcCreateSaveFileInit(uint8_t *rdram, R5900Context *ctx,
                          PS2Runtime *runtime) {
...
void mcDisplayFileSelectWindow(uint8_t *rdram, R5900Context *ctx,
                               PS2Runtime *runtime) {
  TODO_NAMED("mcDisplayFileSelectWindow", rdram, ctx, runtime);
}

void mcDisplaySelectFileInfo(uint8_t *rdram, R5900Context *ctx,
                             PS2Runtime *runtime) {
  TODO_NAMED("mcDisplaySelectFileInfo", rdram, ctx, runtime);
}

void mcDisplaySelectFileInfoMesCount(uint8_t *rdram, R5900Context *ctx,
                                     PS2Runtime *runtime) {
  TODO_NAMED("mcDisplaySelectFileInfoMesCount", rdram, ctx, runtime);
}

void mcDispWindowCurSol(uint8_t *rdram, R5900Context *ctx,
                        PS2Runtime *runtime) {
  TODO_NAMED("mcDispWindowCurSol", rdram, ctx, runtime);
}

void mcDispWindowFoundtion(uint8_t *rdram, R5900Context *ctx,
                           PS2Runtime *runtime) {
...
void mceIntrReadFixAlign(uint8_t *rdram, R5900Context *ctx,
                         PS2Runtime *runtime) {
...
void mcGetConfigCapacitySize(uint8_t *rdram, R5900Context *ctx,
                             PS2Runtime *runtime) {
  TODO_NAMED("mcGetConfigCapacitySize", rdram, ctx, runtime);
}

void mcGetFileSelectWindowCursol(uint8_t *rdram, R5900Context *ctx,
                                 PS2Runtime *runtime) {
  TODO_NAMED("mcGetFileSelectWindowCursol", rdram, ctx, runtime);
}

void mcGetFreeCapacitySize(uint8_t *rdram, R5900Context *ctx,
                           PS2Runtime *runtime) {
  TODO_NAMED("mcGetFreeCapacitySize", rdram, ctx, runtime);
}

void mcGetIconCapacitySize(uint8_t *rdram, R5900Context *ctx,
                           PS2Runtime *runtime) {
  TODO_NAMED("mcGetIconCapacitySize", rdram, ctx, runtime);
}

void mcGetIconFileCapacitySize(uint8_t *rdram, R5900Context *ctx,
                               PS2Runtime *runtime) {
  TODO_NAMED("mcGetIconFileCapacitySize", rdram, ctx, runtime);
}

void mcGetPortSelectDirInfo(uint8_t *rdram, R5900Context *ctx,
                            PS2Runtime *runtime) {
  TODO_NAMED("mcGetPortSelectDirInfo", rdram, ctx, runtime);
}

void mcGetSaveFileCapacitySize(uint8_t *rdram, R5900Context *ctx,
                               PS2Runtime *runtime) {
...
void mcMoveFileSelectWindowCursor(uint8_t *rdram, R5900Context *ctx,
                                  PS2Runtime *runtime) {
  TODO_NAMED("mcMoveFileSelectWindowCursor", rdram, ctx, runtime);
}

void mcNewCreateConfigFile(uint8_t *rdram, R5900Context *ctx,
                           PS2Runtime *runtime) {
...
void mcNewCreateSaveFile(uint8_t *rdram, R5900Context *ctx,
                         PS2Runtime *runtime) {
...
void mcReadStartConfigFile(uint8_t *rdram, R5900Context *ctx,
                           PS2Runtime *runtime) {
  TODO_NAMED("mcReadStartConfigFile", rdram, ctx, runtime);
}

void mcReadStartSaveFile(uint8_t *rdram, R5900Context *ctx,
                         PS2Runtime *runtime) {
  TODO_NAMED("mcReadStartSaveFile", rdram, ctx, runtime);
}

void mcSelectFileInfoInit(uint8_t *rdram, R5900Context *ctx,
                          PS2Runtime *runtime) {
  TODO_NAMED("mcSelectFileInfoInit", rdram, ctx, runtime);
}

void mcSelectSaveFileCheck(uint8_t *rdram, R5900Context *ctx,
                           PS2Runtime *runtime) {
  TODO_NAMED("mcSelectSaveFileCheck", rdram, ctx, runtime);
}

void mcSetFileSelectWindowCursol(uint8_t *rdram, R5900Context *ctx,
                                 PS2Runtime *runtime) {
  TODO_NAMED("mcSetFileSelectWindowCursol", rdram, ctx, runtime);
}

void mcSetFileSelectWindowCursolInit(uint8_t *rdram, R5900Context *ctx,
                                     PS2Runtime *runtime) {
  TODO_NAMED("mcSetFileSelectWindowCursolInit", rdram, ctx, runtime);
}

void mcSetStringSaveFile(uint8_t *rdram, R5900Context *ctx,
                         PS2Runtime *runtime) {
  TODO_NAMED("mcSetStringSaveFile", rdram, ctx, runtime);
}

void mcSetTyepWriteMode(uint8_t *rdram, R5900Context *ctx,
                        PS2Runtime *runtime) {
...
void mcWriteStartConfigFile(uint8_t *rdram, R5900Context *ctx,
                            PS2Runtime *runtime) {
  TODO_NAMED("mcWriteStartConfigFile", rdram, ctx, runtime);
}

void mcWriteStartSaveFile(uint8_t *rdram, R5900Context *ctx,
                          PS2Runtime *runtime) {
