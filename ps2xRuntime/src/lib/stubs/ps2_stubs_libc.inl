...
void realloc(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime) {
  ...
  const uint32_t newGuestAddr =
      runtime ? runtime->guestRealloc(oldGuestAddr, newSize) : 0u;
...
void memcpy(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime) {
  ...
  if (hostDest && hostSrc) {
    ...
    ps2TraceGuestRangeWrite(rdram, destAddr, static_cast<uint32_t>(size),
                            "memcpy", ctx);
  } else {
    std::cerr << "memcpy error: Attempted copy involving non-RDRAM address (or "
                 "invalid RDRAM address)."
              << " Dest: 0x" << std::hex << destAddr
              << " (host ptr valid: " << (hostDest != nullptr) << ")"
              << ", Src: 0x" << srcAddr
              << " (host ptr valid: " << (hostSrc != nullptr) << ")" << std::dec
              << ", Size: " << size << std::endl;
  }
...
void memmove(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime) {
  ...
  if (hostDest && hostSrc) {
    ...
    std::cerr << "memmove error: Attempted move involving potentially invalid "
                 "RDRAM address."
              << " Dest: 0x" << std::hex << destAddr
              << " (host ptr valid: " << (hostDest != nullptr) << ")"
              << ", Src: 0x" << srcAddr
...
void memcmp(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime) {
  ...
  if (hostPtr1 && hostPtr2) {
    ...
    std::cerr << "memcmp error: Invalid address provided."
              << " Ptr1: 0x" << std::hex << ptr1Addr
              << " (host ptr valid: " << (hostPtr1 != nullptr) << ")"
              << ", Ptr2: 0x" << ptr2Addr
              << " (host ptr valid: " << (hostPtr2 != nullptr) << ")"
              << std::dec << std::endl;
...
void strcpy(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime) {
  ...
  const char *hostSrc =
      reinterpret_cast<const char *>(getConstMemPtr(rdram, srcAddr));

  if (hostDest && hostSrc) {
    ::strcpy(hostDest, hostSrc);
    ps2TraceGuestRangeWrite(rdram, destAddr,
...
void strlen(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime) {
  ...
  const char *hostStr =
      reinterpret_cast<const char *>(getConstMemPtr(rdram, strAddr));
  size_t len = 0;

  if (hostStr) {
    len = ::strlen(hostStr);
  } else {
    std::cerr << "strlen error: Invalid address provided: 0x" << std::hex
              << strAddr << std::dec << std::endl;
  }
...
void strcmp(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime) {
  ...
  const char *hostStr1 =
      reinterpret_cast<const char *>(getConstMemPtr(rdram, str1Addr));
  const char *hostStr2 =
      reinterpret_cast<const char *>(getConstMemPtr(rdram, str2Addr));
  int result = 0;
  ...
  if (hostStr1 && hostStr2) {
    ...
    std::cerr << "strcmp error: Invalid address provided."
              << " Str1: 0x" << std::hex << str1Addr
              << " (host ptr valid: " << (hostStr1 != nullptr) << ")"
              << ", Str2: 0x" << str2Addr
              << " (host ptr valid: " << (hostStr2 != nullptr) << ")"
              << std::dec << std::endl;
...
void strchr(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime) {
  ...
  if (hostStr) {
    ...
    std::cerr << "strchr error: Invalid address provided: 0x" << std::hex
              << strAddr << std::dec << std::endl;
  }
...
void strrchr(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime) {
  ...
  if (hostStr) {
    foundPtr = ::strrchr(const_cast<char *>(hostStr),
                         char_code); // Use const_cast carefully
    if (foundPtr) {
      resultAddr = hostPtrToPs2Addr(rdram, foundPtr);
    }
  } else {
    std::cerr << "strrchr error: Invalid address provided: 0x" << std::hex
              << strAddr << std::dec << std::endl;
  }
...
void strstr(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime) {
  ...
  const char *hostHaystack =
      reinterpret_cast<const char *>(getConstMemPtr(rdram, haystackAddr));
  const char *hostNeedle =
      reinterpret_cast<const char *>(getConstMemPtr(rdram, needleAddr));
  char *foundPtr = nullptr;
  uint32_t resultAddr = 0;
  ...
  if (hostHaystack && hostNeedle) {
    ...
    std::cerr << "strstr error: Invalid address provided."
              << " Haystack: 0x" << std::hex << haystackAddr
              << " (host ptr valid: " << (hostHaystack != nullptr) << ")"
              << ", Needle: 0x" << needleAddr
              << " (host ptr valid: " << (hostNeedle != nullptr) << ")"
              << std::dec << std::endl;
  }
...
void printf(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime) {
  ...
  if (format_addr != 0) {
    std::string rendered =
        formatPs2StringWithArgs(rdram, ctx, runtime, formatOwned.c_str(), 1);
    ...
    std::cerr << "printf error: Invalid format string address provided: 0x"
...
void sprintf(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime) {
  ...
  if (format_addr != 0) {
    ...
    if (touchesWatch && watchSprintfLogCount < 64u) {
      ...
      std::cout
          << "[watch:sprintf] dest=0x" << std::hex << str_addr << " fmt@0x"
          << format_addr << " arg0=0x" << arg0 << " arg1=0x" << arg1
          << " fmt=\""
          << sanitizeForLog(
                 readPs2CStringBounded(rdram, runtime, format_addr, 64))
          << "\""
          << " s0=\""
          << sanitizeForLog(readPs2CStringBounded(rdram, runtime, arg0, 64))
          << "\""
          << " s1=\""
          << sanitizeForLog(readPs2CStringBounded(rdram, runtime, arg1, 64))
          << "\"" << std::dec << std::endl;
    ...
    if (writeGuestBytes(rdram, runtime, str_addr,
                        ...
                        writeLen)) {
      ps2TraceGuestRangeWrite(rdram, str_addr, static_cast<uint32_t>(writeLen),
                              "sprintf", ctx);
      ret = static_cast<int>(rendered.size());
    } else {
  ...
  // returns the number of characters written (excluding null), or negative on
  // error.
  setReturnS32(ctx, ret);
}
...
void snprintf(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime) {
  ...
  if (format_addr != 0) {
    std::string rendered =
        formatPs2StringWithArgs(rdram, ctx, runtime, formatOwned.c_str(), 3);
    ret = static_cast<int>(rendered.size());
    ...
    if (size > 0) {
      ...
      if (writeGuestBytes(rdram, runtime, str_addr, output.data(),
                          output.size())) {
        ps2TraceGuestRangeWrite(rdram, str_addr,
    ...
    std::cerr << "snprintf error: Invalid address provided or size is zero."
              << " Dest: 0x" << std::hex << str_addr << ", Format: 0x"
              << format_addr << std::dec << ", Size: " << size << std::endl;
  }
...
void puts(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime) {
  ...
  if (hostStr) {
    ...
    std::cerr << "puts error: Invalid address provided: 0x" << std::hex
              << strAddr << std::dec << std::endl;
  }

  // returns non-negative on success, EOF on error.
  setReturnS32(ctx,
               result >= 0 ? 0 : -1); // PS2 might expect 0/-1 rather than EOF
}
...
void fopen(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime) {
  ...
  const char *hostMode =
      reinterpret_cast<const char *>(getConstMemPtr(rdram, modeAddr));
  ...
  if (hostPath && hostMode) {
    ...
    std::cout << "ps2_stub fopen: path='" << hostPath << "', mode='" << hostMode
              << "'" << std::endl;
    ...
    if (fp) {
      ...
      std::cout << "  -> handle=0x" << std::hex << file_handle << std::dec
                << std::endl;
    } else {
    ...
    std::cerr << "fopen error: Invalid address provided for path or mode."
              << " Path: 0x" << std::hex << pathAddr
              << " (host ptr valid: " << (hostPath != nullptr) << ")"
              << ", Mode: 0x" << modeAddr
              << " (host ptr valid: " << (hostMode != nullptr) << ")"
              << std::dec << std::endl;
  }
...
void fclose(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime) {
  ...
  if (file_handle != 0) {
    ...
    if (it != g_file_map.end()) {
      ...
      std::cerr << "ps2_stub fclose error: Invalid file handle 0x" << std::hex
                << file_handle << std::dec << std::endl;
    }
  } else {
...
void fread(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime) {
  ...
  if (hostPtr && fp && size > 0 && count > 0) {
    ...
    std::cerr << "fread error: Invalid arguments."
              << " Ptr: 0x" << std::hex << ptrAddr
              << " (host ptr valid: " << (hostPtr != nullptr) << ")"
              << ", Handle: 0x" << file_handle
              << " (file valid: " << (fp != nullptr) << ")" << std::dec
...
void fseek(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime) {
  uint32_t file_handle = getRegU32(ctx, 4); // $a0
  long offset =
      (long)getRegU32(ctx, 5); // $a1 (Note: might need 64-bit for large files?)
  ...
  if (fp) {
    ...
    std::cerr << "fseek error: Invalid file handle 0x" << std::hex
              << file_handle << std::dec << std::endl;
  }
...
void ftell(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime) {
  ...
  if (fp) {
    ...
    std::cerr << "ftell error: Invalid file handle 0x" << std::hex
              << file_handle << std::dec << std::endl;
  }
...
void fflush(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime) {
  ...
  if (file_handle == 0) {
    ...
    if (fp) {
      ...
      std::cerr << "fflush error: Invalid file handle 0x" << std::hex
                << file_handle << std::dec << std::endl;
    }
  }
  // returns 0 on success, EOF on error.
...
void __ieee754_rem_pio2f(uint8_t *rdram, R5900Context *ctx,
                         PS2Runtime *runtime) {
  const float x = ctx->f[12];
  ...
  if (float *yOut0 = reinterpret_cast<float *>(getMemPtr(rdram, yOutAddr));
      yOut0) {
    *yOut0 = y0;
  }
  if (float *yOut1 = reinterpret_cast<float *>(getMemPtr(rdram, yOutAddr + 4));
      yOut1) {
    *yOut1 = y1;
  }
