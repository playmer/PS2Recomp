...
std::filesystem::path autoDetectCdImagePath() {
  ...
  for (const std::filesystem::path &root : roots) {
    ...
    std::error_code sizeEc;
    const uint64_t size =
        static_cast<uint64_t>(std::filesystem::file_size(candidate, sizeEc));
  ...
  if (!bestPath.empty()) {
    std::cout << "[CD] Auto-detected disc image: " << bestPath.string()
              << std::endl;
  }
  return bestPath;
}
...
bool tryGetCdImageTotalSectors(uint64_t &totalSectorsOut) {
  ...
  if (!g_cdImageSizeValid || g_cdImageSizePath != imagePath) {
    std::error_code ec;
    g_cdImageSizeBytes =
        static_cast<uint64_t>(std::filesystem::file_size(imagePath, ec));
...
bool resolveCaseInsensitivePath(const std::filesystem::path &root,
  ...
  for (const auto &component : relative) {
    ...
    for (const auto &entry :
         std::filesystem::directory_iterator(current, iterEc)) {
      if (iterEc) {
        break;
      }
...
bool readHostRange(const std::filesystem::path &path, uint64_t offsetBytes,
...
bool readCdSectors(uint32_t lbn, uint32_t sectors, uint8_t *dst,
                   size_t byteCount) {
...
bool writeCdSearchResult(uint8_t *rdram, uint32_t fileAddr,
                         const std::string &ps2Path, const CdFileEntry &entry) {
...
bool tryRemapGdInitSearchToAfs(const std::string &ps2Path, uint32_t callerRa,
...
uint32_t hostPtrToPs2Addr(uint8_t *rdram, const void *hostPtr) {
  ...
  if (offset >= 0 && static_cast<size_t>(offset) < PS2_RAM_SIZE) {
    ...
    std::cerr << "Warning: hostPtrToPs2Addr failed - host pointer " << hostPtr
              << " is outside rdram range [" << static_cast<void *>(rdram)
              << ", " << static_cast<void *>(rdram + PS2_RAM_SIZE) << ")"
              << std::endl;
    return 0;
  }
}
...
bool tryReadWordFromGuest(uint8_t *rdram, PS2Runtime *runtime, uint32_t addr,
                          uint32_t &outWord) {
...
bool tryReadByteFromGuest(uint8_t *rdram, PS2Runtime *runtime, uint32_t addr,
                          uint8_t &outByte) {
...
bool writeGuestBytes(uint8_t *rdram, PS2Runtime *runtime, uint32_t addr,
...
std::string readPs2CStringBounded(uint8_t *rdram, PS2Runtime *runtime,
                                  uint32_t addr, size_t maxLen = 512) {
  std::string out;
  if (addr == 0 || maxLen == 0) {
    return out;
  }
  ...
  for (size_t i = 0; i < maxLen; ++i) {
    ...
    uint8_t chByte = 0;
    if (!tryReadByteFromGuest(rdram, runtime, static_cast<uint32_t>(guestAddr),
                              chByte)) {
      break;
    }
...
std::string readPs2CStringBounded(uint8_t *rdram, uint32_t addr,
                                  size_t maxLen = 512) {
...
public:
  Ps2VarArgCursor(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime,
                  int fixedArgs)
      : m_rdram(rdram), m_ctx(ctx), m_runtime(runtime), m_fixedArgs(fixedArgs),
...
std::string formatPs2StringCore(uint8_t *rdram, const char *format,
                                NextU32Fn nextU32, NextU64Fn nextU64,
  ...
  while (*p) {
    ...
    enum class LengthMod { None, H, HH, L, LL, J, Z, T, BigL };
    ...
    const bool use64Integer =
        (length == LengthMod::LL || length == LengthMod::J);
...
std::string formatPs2StringWithArgs(uint8_t *rdram, R5900Context *ctx,
                                    PS2Runtime *runtime, const char *format,
                                    int fixedArgs) {
  Ps2VarArgCursor cursor(rdram, ctx, runtime, fixedArgs);
  return formatPs2StringCore(
      rdram, format, [&cursor]() { return cursor.nextU32(); },
      [&cursor]() { return cursor.nextU64(); },
      [rdram, runtime](uint32_t addr) {
        return readPs2CStringBounded(rdram, runtime, addr);
      });
}

std::string formatPs2StringWithVaList(uint8_t *rdram, PS2Runtime *runtime,
                                      const char *format, uint32_t vaListAddr) {
...
bool isKnownDmaChannelBase(uint32_t value) {
  return std::find(kDmaChannelBases.begin(), kDmaChannelBases.end(), value) !=
         kDmaChannelBases.end();
}

uint32_t toDmaPhys(uint32_t addr) { return addr & 0x1FFFFFFFu; }
...
int32_t submitDmaSend(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime,
                      bool preferNormalCount) {
  if (!runtime) {
    return -1;
  }
  ...
  if (g_dmaStubLogCount < kMaxDmaStubLogs) {
    std::cout << "[sceDmaSend] ch=0x" << std::hex << channelBase << " madr=0x"
              << madr << " qwc=0x" << qwc << " tadr=0x" << tadr << " chcr=0x"
              << chcr << std::dec << std::endl;
    ++g_dmaStubLogCount;
  }
...
static uint64_t makePmode(uint32_t en1, uint32_t en2, uint32_t mmod,
                          uint32_t amod, uint32_t slbg, uint32_t alp) {
...
static uint64_t makeDispFb(uint32_t fbp, uint32_t fbw, uint32_t psm,
                           uint32_t dbx, uint32_t dby) {
...
static uint64_t makeDisplay(uint32_t dx, uint32_t dy, uint32_t magh,
                            uint32_t magv, uint32_t dw, uint32_t dh) {
...
static uint32_t readStackU32(uint8_t *rdram, R5900Context *ctx,
                             uint32_t offset) {
...
static GsSetDefImageArgs decodeGsSetDefImageArgs(uint8_t *rdram,
                                                 R5900Context *ctx) {
...
static bool writeGsDispEnv(uint8_t *rdram, uint32_t addr, uint64_t display,
                           uint64_t dispfb) {
  uint8_t *ptr = getMemPtr(rdram, addr);
  if (!ptr)
    return false;
