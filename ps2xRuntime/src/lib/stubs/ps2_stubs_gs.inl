void sceGsExecLoadImage(uint8_t *rdram, R5900Context *ctx,
                        PS2Runtime *runtime) {
  ...
  const uint32_t rowBytes =
      bytesForPixels(img.psm, static_cast<uint32_t>(img.width));
  if (rowBytes == 0) {
    setReturnS32(ctx, -1);
    return;
  }

  uint32_t fbw = img.vram_width ? img.vram_width
                                : std::max<uint32_t>(1, (img.width + 63) / 64);
  ...
  if (logCount < 8) {
    std::cout << "ps2_stub sceGsExecLoadImage: x=" << img.x << " y=" << img.y
              << " w=" << img.width << " h=" << img.height << " vram=0x"
              << std::hex << img.vram_addr << " fbw=" << std::dec
              << static_cast<int>(fbw) << " psm=" << static_cast<int>(img.psm)
              << " src=0x" << std::hex << srcAddr << std::dec << std::endl;
    ++logCount;
  }

  for (uint32_t row = 0; row < img.height; ++row) {
    uint32_t dstOff = base + (static_cast<uint32_t>(img.y) + row) * stride +
                      bytesForPixels(img.psm, static_cast<uint32_t>(img.x));
...
void sceGsExecStoreImage(uint8_t *rdram, R5900Context *ctx,
                         PS2Runtime *runtime) {
  uint32_t imgAddr = getRegU32(ctx, 4);
  ...
  static int logCount = 0;
  if (logCount < 8) {
    std::cout << "ps2_stub sceGsExecStoreImage: x=" << img.x << " y=" << img.y
              << " w=" << img.width << " h=" << img.height << " vram=0x"
              << std::hex << img.vram_addr << " fbw=" << std::dec
              << static_cast<int>(fbw) << " psm=" << static_cast<int>(img.psm)
  ...
  for (uint32_t row = 0; row < img.height; ++row) {
    uint32_t srcOff = base + (static_cast<uint32_t>(img.y) + row) * stride +
                      bytesForPixels(img.psm, static_cast<uint32_t>(img.x));
...
void sceGsSetDefDBuffDc(uint8_t *rdram, R5900Context *ctx,
                        PS2Runtime *runtime) {
  setReturnS32(ctx, 0);
}

void sceGsSetDefDispEnv(uint8_t *rdram, R5900Context *ctx,
                        PS2Runtime *runtime) {
  uint32_t envAddr = getRegU32(ctx, 4);
  uint32_t psm = getRegU32(ctx, 5);
  uint32_t w = getRegU32(ctx, 6);
...
void sceGsSetDefDrawEnv(uint8_t *rdram, R5900Context *ctx,
                        PS2Runtime *runtime) {
  TODO_NAMED("sceGsSetDefDrawEnv", rdram, ctx, runtime);
}

void sceGsSetDefDrawEnv2(uint8_t *rdram, R5900Context *ctx,
                         PS2Runtime *runtime) {
  TODO_NAMED("sceGsSetDefDrawEnv2", rdram, ctx, runtime);
}

void sceGsSetDefLoadImage(uint8_t *rdram, R5900Context *ctx,
                          PS2Runtime *runtime) {
  uint32_t imgAddr = getRegU32(ctx, 4);
...
void sceGsSetDefStoreImage(uint8_t *rdram, R5900Context *ctx,
                           PS2Runtime *runtime) {
...
void sceGsSyncVCallback(uint8_t *rdram, R5900Context *ctx,
                        PS2Runtime *runtime) {
  setReturnS32(ctx, 0);
}
