...
std::optional<PS2Runtime::RecompiledFunction>
resolveHandlerByName(std::string_view handlerName) {
  const std::string_view resolvedSyscall =
      ps2_runtime_calls::resolveSyscallName(handlerName);
...
#undef PS2_RESOLVE_SYSCALL
  ...
  const std::string_view resolvedStub =
      ps2_runtime_calls::resolveStubName(handlerName);
...
void registerDescriptor(const Descriptor &descriptor) {
  if (!descriptor.apply) {
    std::cerr
        << "[game_overrides] ignoring descriptor with null apply callback."
        << std::endl;
    return;
  }
...
bool bindAddressHandler(PS2Runtime &runtime, uint32_t address,
...
void applyMatching(PS2Runtime &runtime, const std::string &elfPath,
                   uint32_t entry) {
  ...
  for (const Descriptor &descriptor : descriptors) {
    ...
    if (descriptor.crc32 != 0u) {
      if (!fileCrcComputed) {
        ...
        if (!fileCrcValid) {
          std::cerr << "[game_overrides] failed to compute CRC32 for '"
                    << elfPath << "'" << std::endl;
        }
      }
  ...
  if (appliedCount > 0) {
    std::cout << "[game_overrides] applied " << appliedCount
              << " matching override(s)." << std::endl;
  }
}
} // namespace ps2_game_overrides
