...
static void writeGuestU32NoThrow(uint8_t *rdram, uint32_t addr,
                                 uint32_t value) {
  if (addr == 0u) {
    return;
  }
...
static void writeGuestU64NoThrow(uint8_t *rdram, uint32_t addr,
                                 uint64_t value) {
  if (addr == 0u) {
    return;
  }
...
static void dispatchIntcHandlersForCause(uint8_t *rdram, PS2Runtime *runtime,
                                         uint32_t cause) {
  if (!rdram || !runtime) {
    return;
  }
  ...
  for (const IrqHandlerInfo &info : handlers) {
    ...
    try {
      ...
      PS2Runtime::RecompiledFunction func =
          runtime->lookupFunction(info.handler);
...
void stopInterruptWorker() {
  ...
  for (int i = 0;
       i < 100 && g_irq_worker_running.load(std::memory_order_acquire); ++i) {
...
void DisableIntcHandler(uint8_t *rdram, R5900Context *ctx,
                        PS2Runtime *runtime) {
  const int handlerId = static_cast<int>(getRegU32(ctx, 5));
  {
    std::lock_guard<std::mutex> lock(g_irq_handler_mutex);
...
void DisableDmacHandler(uint8_t *rdram, R5900Context *ctx,
                        PS2Runtime *runtime) {
  const int handlerId = static_cast<int>(getRegU32(ctx, 5));
  {
    std::lock_guard<std::mutex> lock(g_irq_handler_mutex);
