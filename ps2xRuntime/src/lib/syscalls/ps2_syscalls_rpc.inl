void SifStopModule(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime) {
  ...
  if (resultAddr != 0) {
    int32_t *hostResult =
        reinterpret_cast<int32_t *>(getMemPtr(rdram, resultAddr));
    if (hostResult) {
...
void SifLoadModule(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime) {
  ...
  const std::string modulePath =
      readGuestCStringBounded(rdram, pathAddr, kMaxSifModulePathBytes);
  if (modulePath.empty()) {
    setReturnS32(ctx, -1);
    return;
  }
...
void SifBindRpc(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime) {
  ...
  t_SifRpcClientData *client =
      reinterpret_cast<t_SifRpcClientData *>(getMemPtr(rdram, clientPtr));
...
void SifCallRpc(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime) {
  ...
  const bool isDtxUrpc =
      (sid == kDtxRpcSid) && (rpcNum >= 0x400u) && (rpcNum < 0x500u);
  ...
  if (isDtxUrpc) {
    ...
    if (dtxUrpcCommand < 64u) {
      ...
      (void)readRpcU32(kDtxUrpcObjTableBase + (dtxUrpcCommand * 4u),
                       dtxUrpcObj);
    }
  ...
  if (sd && sd->func && (sid != kDtxRpcSid || isDtxUrpc) &&
      allowServerDispatch) {
    dtxUrpcDispatchAttempted = dtxUrpcDispatchAttempted || isDtxUrpc;
    handled = rpcInvokeFunction(rdram, ctx, runtime, sd->func, rpcNum, sd->buf,
                                sendSize, 0, &resultPtr);
  ...
  if (!handled && isDtxUrpc && sendBuf && sendSize > 0) {
    // Only dispatch through dtx_rpc_func when a URPC handler is registered in
    // the table. If the slot is empty, defer to the fallback emulation below.
    if (hasUrpcHandler) {
      dtxUrpcDispatchAttempted = true;
      handled = rpcInvokeFunction(rdram, ctx, runtime, 0x2fabc0u, rpcNum,
                                  sendBuf, sendSize, 0, &resultPtr);
    ...
    if (rpcNum == 2 && recvBuf && recvSize >= sizeof(uint32_t)) {
      ...
      uint32_t remoteHandle = 0;
      {
        ...
        if (!remoteHandle) {
          ...
          if (!remoteHandle) {
            remoteHandle =
                kRpcServerPoolBase + ((dtxId & 0xFFu) * kRpcServerStride);
          }
      ...
      uint32_t remoteHandle = 0;
      if (sendBuf && sendSize >= sizeof(uint32_t) &&
          readRpcU32(sendBuf, remoteHandle) && remoteHandle) {
        std::lock_guard<std::mutex> lock(g_dtx_rpc_mutex);
        for (auto it = g_dtx_remote_by_id.begin();
             it != g_dtx_remote_by_id.end(); ++it) {
      ...
      case 37u: // SJRMT_RESET
      {
        ...
        if (it != g_dtx_sjrmt_by_handle.end()) {
          const uint32_t cap =
              (it->second.wkSize == 0u) ? 0x4000u : it->second.wkSize;
      ...
      case 41u: // SJRMT_GET_NUM_DATA
      {
        ...
        if (it != g_dtx_sjrmt_by_handle.end()) {
          outWords[0] =
              (streamId == 0u) ? it->second.roomBytes : it->second.dataBytes;
        } else {
          outWords[0] = 0u;
        }
        outWordCount = 1u;
      ...
      if (recvBuf && recvSize > 0u) {
        const uint32_t recvWordCapacity =
            static_cast<uint32_t>(recvSize / sizeof(uint32_t));
  ...
  if (isDtxUrpc) {
    ...
    if (dtxUrpcLogCount < 64) {
      ...
      std::cout << "[SifCallRpc:DTX] rpcNum=0x" << std::hex << rpcNum
                << " cmd=0x" << dtxUrpcCommand << " fn=0x" << dtxUrpcFn
                << " obj=0x" << dtxUrpcObj << " send0=0x" << dtxUrpcSend0
                << " recv0=0x" << dtxUrpcRecv0 << " resultPtr=0x" << resultPtr
                << " handled=" << std::dec << (handled ? 1 : 0)
                << " dispatch=" << (dtxUrpcDispatchAttempted ? 1 : 0)
                << " emu=" << (dtxUrpcFallbackEmulated ? 1 : 0)
                << " emu34=" << (dtxUrpcFallbackCreate34 ? 1 : 0) << std::endl;
      ++dtxUrpcLogCount;
    }
  }

  if (endFunc) {
    bool callbackInvoked = rpcInvokeFunction(rdram, ctx, runtime, endFunc,
                                             endParam, 0, 0, 0, nullptr);
    ...
    if (!callbackInvoked && (endFunc == 0x2fac20u || endFunc == 0x2fac30u)) {
      ...
      callbackInvoked = rpcInvokeFunction(
          rdram, ctx, runtime, normalizedEndFunc, endParam, 0, 0, 0, nullptr);
    }
    ...
    const bool isSoundRpcCallback =
        (endFunc == 0x2eac20u || endFunc == 0x2eac30u || endFunc == 0x2fac20u ||
         endFunc == 0x2fac30u);
    if (isSoundRpcCallback) {
      (void)signalRpcCompletionSema(endParam);
      if (rdram && (endFunc == 0x2eac30u || endFunc == 0x2fac30u)) {
        constexpr uint32_t kSndBusyFlagAddr = 0x01E212C8u;
        if (uint32_t *busy = reinterpret_cast<uint32_t *>(
                getMemPtr(rdram, kSndBusyFlagAddr))) {
          *busy = 0u;
        }
      }
    }
    ...
    if (!callbackInvoked) {
      ...
      if (unresolvedEndFuncWarnCount < 32u) {
        std::cerr << "[SifCallRpc] unresolved end callback endFunc=0x"
                  << std::hex << endFunc << " endParam=0x" << endParam
                  << " fallbackSignal=" << std::dec
                  << (fallbackSignaledSema ? 1 : 0) << std::endl;
  ...
  if (logCount < 10) {
    std::cout << "[SifCallRpc] client=0x" << std::hex << clientPtr << " sid=0x"
              << sid << " rpcNum=0x" << rpcNum << " mode=0x" << mode
              << " sendBuf=0x" << sendBuf << " recvBuf=0x" << recvBuf
              << " recvSize=0x" << recvSize << " size=" << std::dec << sendSize
              << std::endl;
    ++logCount;
  }
...
void SifRegisterRpc(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime) {
  ...
  t_SifRpcServerData *sd =
      reinterpret_cast<t_SifRpcServerData *>(getMemPtr(rdram, sdPtr));
  if (!sd) {
    setReturnS32(ctx, -1);
    return;
  }
  ...
  if (qd) {
    t_SifRpcDataQueue *queue =
        reinterpret_cast<t_SifRpcDataQueue *>(getMemPtr(rdram, qd));
    if (queue) {
      if (!queue->link) {
        queue->link = sdPtr;
      } else {
        ...
        for (int guard = 0; guard < 1024 && curPtr; ++guard) {
          t_SifRpcServerData *cur =
              reinterpret_cast<t_SifRpcServerData *>(getMemPtr(rdram, curPtr));
          if (!cur)
            break;
          if (!cur->link) {
            cur->link = sdPtr;
...

  {
    ...
    for (auto &entry : g_rpc_clients) {
      if (entry.second.sid == sid) {
        t_SifRpcClientData *cd = reinterpret_cast<t_SifRpcClientData *>(
            getMemPtr(rdram, entry.first));
        if (cd) {
          cd->server = sdPtr;
          cd->buf = sd->buf;
          cd->cbuf = sd->cbuf;
  ...
  std::cout << "[SifRegisterRpc] sid=0x" << std::hex << sid << " sd=0x" << sdPtr
            << std::dec << std::endl;
  setReturnS32(ctx, 0);
}
...
void SifSetRpcQueue(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime) {
  ...
  t_SifRpcDataQueue *qd =
      reinterpret_cast<t_SifRpcDataQueue *>(getMemPtr(rdram, qdPtr));
  if (!qd) {
    setReturnS32(ctx, -1);
    return;
  }
...

  {
    ...
    if (!g_rpc_active_queue) {
      ...
      for (int guard = 0; guard < 1024 && curPtr; ++guard) {
        if (curPtr == qdPtr)
          break;
        t_SifRpcDataQueue *cur =
            reinterpret_cast<t_SifRpcDataQueue *>(getMemPtr(rdram, curPtr));
        if (!cur)
          break;
        if (!cur->next) {
          cur->next = qdPtr;
...
void sceSifSendCmd(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime) {
  ...
  static int logCount = 0;
  if (logCount < 5) {
    std::cout << "[sceSifSendCmd] cid=0x" << std::hex << cid << " packet=0x"
              << packetAddr << " psize=0x" << packetSize << " extra=0x"
              << destExtra << std::dec << std::endl;
    ++logCount;
  }
