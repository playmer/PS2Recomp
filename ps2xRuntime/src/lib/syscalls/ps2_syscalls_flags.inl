...
static bool readGuestU32Safe(const uint8_t *rdram, uint32_t addr,
                             uint32_t &out) {
...
static DecodedSemaParams decodeCreateSemaParams(const uint32_t *param,
  ...
  if (hasLegacyLayout && hasEeLayout && countLooksPlausible(legacyMax) &&
      !countLooksPlausible(eeMax)) {
    useLegacyLayout = true;
  } else if (hasLegacyLayout && hasEeLayout && countLooksPlausible(legacyMax) &&
             countLooksPlausible(eeMax)) {
    ...
    const bool legacyOptionLooksValid =
        looksLikeGuestPointerOrNull(legacyOption);
...
void CreateSema(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime) {
  ...
  const DecodedSemaParams decoded =
      decodeCreateSemaParams(rawParams, availableWords);
  ...
  std::cout << "[CreateSema] id=" << id << " init=" << init << " max=" << max
            << std::endl;
  setReturnS32(ctx, id);
}
...
void ReferSemaStatus(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime) {
  ...
  ee_sema_t *status =
      reinterpret_cast<ee_sema_t *>(getMemPtr(rdram, statusAddr));
  if (!status) {
    setReturnS32(ctx, KE_ERROR);
    return;
  }
...
void WaitEventFlag(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime) {
  ...
  uint32_t *resBitsPtr =
      resBitsAddr ? reinterpret_cast<uint32_t *>(getMemPtr(rdram, resBitsAddr))
                  : nullptr;
...
void ReferEventFlagStatus(uint8_t *rdram, R5900Context *ctx,
  ...
  Ps2EventFlagInfo *out =
      infoAddr
          ? reinterpret_cast<Ps2EventFlagInfo *>(getMemPtr(rdram, infoAddr))
          : nullptr;
  if (!out) {
    setReturnS32(ctx, -1);
    return;
...
void SetAlarm(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime) {
  ...
  if (logCount < 5) {
    std::cout << "[SetAlarm] ticks=" << ticks << " handler=0x" << std::hex
              << handler << " arg=0x" << arg << std::dec << std::endl;
    ++logCount;
  }
