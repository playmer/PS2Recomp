namespace {
std::string readGuestCStringBounded(const uint8_t *rdram, uint32_t guestAddr,
                                    size_t maxBytes) {
  std::string out;
  ...
  for (size_t i = 0; i < maxBytes; ++i) {
    const char ch = static_cast<char>(
        rdram[(guestAddr + static_cast<uint32_t>(i)) & PS2_RAM_MASK]);
    if (ch == '\0') {
      break;
    }
    out.push_back(ch);
...
uint64_t hashGuestBytesFnv1a64(const uint8_t *rdram, uint32_t guestAddr,
                               size_t byteCount) {
...
std::string makeSifModuleBufferTag(const uint8_t *rdram, uint32_t bufferAddr) {
  char key[96] = {};
  const uint64_t hash =
      hashGuestBytesFnv1a64(rdram, bufferAddr, kSifModuleBufferProbeBytes);
  std::snprintf(key, sizeof(key), "iopbuf:fnv64:%016llx",
                static_cast<unsigned long long>(hash));
  return std::string(key);
}

void logSifModuleAction(const char *op, int32_t moduleId,
                        const std::string &path, uint32_t refCount) {
  if (!op) {
    return;
  }
  ...
  std::cout << "[SIF module] " << op << " id=" << moduleId
            << " ref=" << refCount << " path=\"" << path << "\"" << std::endl;
...
bool readFileBlockAt(std::ifstream &file, uint64_t offset, void *dst,
                     size_t byteCount) {
  if (!dst || byteCount == 0) {
    return false;
  }
...
bool tryExtractElfGpValue(std::ifstream &file, const Elf32Header &header,
                          uint32_t &gpOut) {
  uint8_t regInfo[24] = {};

  for (uint32_t i = 0; i < header.phnum; ++i) {
    Elf32ProgramHeader ph{};
    const uint64_t phOffset = static_cast<uint64_t>(header.phoff) +
                              static_cast<uint64_t>(i) * header.phentsize;
  ...
  for (uint32_t i = 0; i < header.shnum; ++i) {
    ...
    const uint64_t shOffset = static_cast<uint64_t>(header.shoff) +
                              static_cast<uint64_t>(i) * header.shentsize;
...
bool loadElfIntoGuestMemory(const std::string &hostPath, uint8_t *rdram,
                            PS2Runtime *runtime, const std::string &sectionName,
                            GuestExecData &execDataOut, std::string &errorOut) {
  ...
  if (header.magic != kElfMagic || header.machine != kElfMachineMips ||
  ...
  const bool loadAll =
      sectionName.empty() || toLowerAscii(sectionName) == "all";
  ...
  for (uint32_t i = 0; i < header.phnum; ++i) {
    ...
    if (runtime && ph.vaddr >= PS2_SCRATCHPAD_BASE &&
        ph.vaddr < (PS2_SCRATCHPAD_BASE + PS2_SCRATCHPAD_SIZE)) {
      const uint32_t scratchOffset =
          runtime->memory().translateAddress(ph.vaddr);
      if (static_cast<uint64_t>(scratchOffset) + memSize64 >
          PS2_SCRATCHPAD_SIZE) {
      ...
      const uint32_t physAddr =
          runtime ? runtime->memory().translateAddress(ph.vaddr)
                  : (ph.vaddr & PS2_RAM_MASK);
...
int32_t runSifLoadElfPart(uint8_t *rdram, R5900Context *ctx,
                          PS2Runtime *runtime, uint32_t pathAddr,
  ...
  const std::string ps2Path =
      readGuestCStringBounded(rdram, pathAddr, kLoadfilePathMaxBytes);
  if (ps2Path.empty()) {
    return -1;
  }
  ...
  if (!loadElfIntoGuestMemory(hostPath, rdram, runtime, sectionName, execData,
                              loadError)) {
    static uint32_t logCount = 0;
    if (logCount < 16u) {
      std::cerr << "[SifLoadElfPart] failed path=\"" << ps2Path << "\" host=\""
                << hostPath << "\" reason=" << loadError << std::endl;
      ++logCount;
    }
    return -1;
  }
  ...
  if (execDataAddr != 0u) {
    GuestExecData *guestExec =
        reinterpret_cast<GuestExecData *>(getMemPtr(rdram, execDataAddr));
    if (!guestExec) {
      return -1;
    }
  ...
  if (successLogs < 16u) {
    std::cout << "[SifLoadElfPart] loaded \"" << ps2Path << "\" epc=0x"
              << std::hex << execData.epc << " gp=0x" << execData.gp << std::dec
              << std::endl;
    ++successLogs;
  }
