...
static std::chrono::microseconds alarmTicksToDuration(uint16_t ticks) {
  ...
  const uint64_t clampedTicks =
      (ticks == 0u) ? 1u : static_cast<uint64_t>(ticks);
      ...
      for (;;) {
        std::shared_ptr<AlarmInfo> readyAlarm;
        {
          ...
          while (!readyAlarm) {
            ...
            auto nextIt =
                std::min_element(g_alarms.begin(), g_alarms.end(),
        ...
        try {
          ...
          if (alarmExceptionLogs < 8) {
            std::cerr << "[SetAlarm] callback exception: " << e.what()
                      << std::endl;
            ++alarmExceptionLogs;
          }
        }
...
static void rpcCopyToRdram(uint8_t *rdram, uint32_t dst, uint32_t src,
                           size_t size) {
  if (!rdram || size == 0)
    return;
  ...
  if (clampedSize != size) {
    ...
    if (warnCount < 8) {
      std::cerr << "[SifCallRpc] clamping copy size from " << size << " to "
                << clampedSize << " bytes (dst=0x" << std::hex << dst
...
static void rpcZeroRdram(uint8_t *rdram, uint32_t dst, size_t size) {
  ...
  if (clampedSize != size) {
    static uint32_t warnCount = 0;
    if (warnCount < 8) {
      std::cerr << "[SifCallRpc] clamping zero size from " << size << " to "
                << clampedSize << " bytes (dst=0x" << std::hex << dst
                << std::dec << ")" << std::endl;
      ++warnCount;
    }
...
static bool readStackU32(uint8_t *rdram, uint32_t sp, uint32_t offset,
                         uint32_t &out) {
...
static bool rpcInvokeFunction(uint8_t *rdram, R5900Context *ctx,
                              PS2Runtime *runtime, uint32_t funcAddr,
                              uint32_t a0, uint32_t a1, uint32_t a2,
                              uint32_t a3, uint32_t *outV0) {
...
std::string translatePs2Path(const char *ps2Path) {
  ...
  auto resolveWithBase = [&](const std::filesystem::path &base,
  ...
  if (lower.rfind("host0:", 0) == 0 || lower.rfind("host:", 0) == 0) {
    ...
    return resolveWithBase(getConfiguredHostRoot(),
...
static int getTimezoneOffsetMinutes() {
  ...
  if (localTime == static_cast<std::time_t>(-1) ||
      gmtTime == static_cast<std::time_t>(-1))
    return 0;
...
static uint32_t packOsdConfig(uint32_t spdifMode, uint32_t screenType,
                              uint32_t videoOutput, uint32_t japLanguage,
                              uint32_t ps1drvConfig, uint32_t version,
...
static uint32_t sanitizeOsdConfigRaw(uint32_t raw) {
  ...
  return packOsdConfig(spdifMode, screenType, videoOutput, japLanguage,
                       ps1drvConfig, version, language, tz);
}
...
static void ensureOsdConfigInitialized() {
  ...
  g_osd_config_raw =
      packOsdConfig(spdifMode, screenType, videoOutput, japLanguage,
                    ps1drvConfig, version, language, tz);
...
static uint32_t createBootModeEntry(uint8_t *rdram, uint8_t id, uint16_t value,
                                    uint8_t lenField, const uint32_t *data,
                                    uint8_t dataCount) {
