...
struct alignas(16) R5900Context {
  ...
  void dump() const {
    ...
    std::cout << "HI: 0x" << std::setw(8) << hi << " LO: 0x" << std::setw(8)
              << lo << "\n";
    std::cout << "HI1:0x" << std::setw(8) << hi1 << " LO1:0x" << std::setw(8)
              << lo1 << "\n";
    std::cout << "SA: 0x" << std::setw(8) << sa << "\n";
    for (int i = 0; i < 32; ++i) {
      std::cout << "R" << std::setw(2) << std::dec << i << ": 0x" << std::hex
                << std::setw(8)
                << static_cast<uint32_t>(_mm_extract_epi32(r[i], 3))
                << std::setw(8)
                << static_cast<uint32_t>(_mm_extract_epi32(r[i], 2)) << "_"
                << std::setw(8)
                << static_cast<uint32_t>(_mm_extract_epi32(r[i], 1))
                << std::setw(8)
                << static_cast<uint32_t>(_mm_extract_epi32(r[i], 0)) << "\n";
    }
    std::cout << "Status: 0x" << std::setw(8) << cop0_status << " Cause: 0x"
              << std::setw(8) << cop0_cause << " EPC: 0x" << std::setw(8)
              << cop0_epc << "\n";
...
inline void setReturnU64(R5900Context *ctx, uint64_t value) {
  ...
  ctx->r[3] = _mm_set_epi64x(
      0, static_cast<int64_t>(static_cast<uint32_t>(value >> 32)));
}
...
inline bool ps2PathWatchIntersects(uint32_t writeAddr, uint32_t writeSize) {
  ...
  const uint64_t watchEnd =
      watchStart + static_cast<uint64_t>(PS2_PATH_WATCH_BYTES);
...
inline uint8_t ps2PathWatchExtractByteFromWrite(uint32_t writeAddr,
                                                uint32_t watchAddr,
                                                uint64_t valueLo,
                                                uint64_t valueHi) {
...
inline void ps2TraceGuestWrite(uint8_t *rdram, uint32_t guestAddr,
                               uint32_t size, uint64_t valueLo,
                               uint64_t valueHi, const char *op,
  ...
  const uint32_t logIndex =
      g_ps2PathWatchLogCount.fetch_add(1, std::memory_order_relaxed);
  if (logIndex >= PS2_PATH_WATCH_MAX_LOGS) {
    return;
  }

  const uint32_t watchAddr = ps2PathWatchPhysAddr();
  const bool touchesFirstByte =
      (watchAddr >= writeAddr) && (watchAddr < writeAddr + size);
  const uint8_t oldByte = rdram[watchAddr];
  const uint8_t newByte =
      touchesFirstByte ? ps2PathWatchExtractByteFromWrite(writeAddr, watchAddr,
                                                          valueLo, valueHi)
                       : oldByte;
  ...
  auto flags = std::cout.flags();
  std::cout << "[watch:path-write] #" << (logIndex + 1u) << " op=" << op
            << " addr=0x" << std::hex << writeAddr << " size=0x" << size
            << " pc=0x" << pc << " ra=0x" << ra << " sp=0x" << sp << " vLo=0x"
            << valueLo;
  if (size > 8u) {
...
inline void ps2TraceGuestRangeWrite(uint8_t *rdram, uint32_t guestAddr,
  ...
  auto flags = std::cout.flags();
  std::cout << "[watch:path-range] #" << (logIndex + 1u) << " op=" << op
            << " addr=0x" << std::hex << writeAddr << " size=0x" << size
            << " pc=0x" << pc << " ra=0x" << ra << " sp=0x" << sp
...
public:
  ...
  void executeVU0Microprogram(uint8_t *rdram, R5900Context *ctx,
                              uint32_t address);
  void vu0StartMicroProgram(uint8_t *rdram, R5900Context *ctx,
                            uint32_t address);
...
public:
  ...
  void configureGuestHeap(uint32_t guestBase,
                          uint32_t guestLimit = PS2_RAM_SIZE);
  ...
  uint32_t guestRealloc(uint32_t guestAddr, uint32_t newSize,
  ...
  void Store16(uint8_t *rdram, R5900Context *ctx, uint32_t vaddr,
               uint16_t value);
  void Store32(uint8_t *rdram, R5900Context *ctx, uint32_t vaddr,
               uint32_t value);
  void Store64(uint8_t *rdram, R5900Context *ctx, uint32_t vaddr,
               uint64_t value);
  void Store128(uint8_t *rdram, R5900Context *ctx, uint32_t vaddr,
                __m128i value);
  ...
  static inline bool isSpecialAddress(uint32_t addr) {
    ...
    // Scratchpad (16KB)
    if (addr >= PS2_SCRATCHPAD_BASE &&
        addr < (PS2_SCRATCHPAD_BASE + PS2_SCRATCHPAD_SIZE))
      return true;
    ...
    if (addr >= PS2_VU0_CODE_BASE &&
        addr < (PS2_VU1_DATA_BASE + PS2_VU1_DATA_SIZE))
      return true;
