...
constexpr uint32_t PS2_IO_BASE =
    0x10000000; // Base for many I/O regs (Timers, DMAC, INTC)
...
constexpr uint32_t PS2_VU0_CODE_BASE =
    0x11000000; // Base address as seen from EE
...
constexpr uint32_t PS2_VU1_MEM_BASE =
    PS2_VU1_CODE_BASE; // Alias used by older code paths
...
constexpr uint32_t PS2_GS_PRIV_REG_BASE =
    PS2_GS_BASE; // GS Privileged Registers
...
static_assert((PS2_RAM_SIZE & (PS2_RAM_SIZE - 1u)) == 0u,
              "PS2_RAM_SIZE must be a power of two");
static_assert(PS2_RAM_MASK == (PS2_RAM_SIZE - 1u),
              "PS2_RAM_MASK must match PS2_RAM_SIZE");
...
inline bool ps2ResolveGuestPointer(uint32_t addr, uint32_t &offset,
                                   bool &scratch) {
...
static_assert(sizeof(GSRegisters) == (19u * sizeof(uint64_t)),
              "GSRegisters layout changed unexpectedly");
static_assert(alignof(GSRegisters) == alignof(uint64_t),
              "GSRegisters alignment must remain 64-bit");
...
static_assert(sizeof(VIFRegisters) == (23u * sizeof(uint32_t)),
              "VIFRegisters layout changed unexpectedly");
...
static_assert(sizeof(DMARegisters) == (7u * sizeof(uint32_t)),
              "DMARegisters layout changed unexpectedly");
...
public:
  ...
  uint64_t dmaStartCount() const {
    return m_dmaStartCount.load(std::memory_order_relaxed);
  }
  uint64_t gifCopyCount() const {
    return m_gifCopyCount.load(std::memory_order_relaxed);
  }
  uint64_t gsWriteCount() const {
    return m_gsWriteCount.load(std::memory_order_relaxed);
  }
  uint64_t vifWriteCount() const {
    return m_vifWriteCount.load(std::memory_order_relaxed);
  }
  ...
  bool tlbRead(uint32_t index, uint32_t &vpn, uint32_t &pfn, uint32_t &mask,
               bool &valid) const;
  bool tlbWrite(uint32_t index, uint32_t vpn, uint32_t pfn, uint32_t mask,
                bool valid);
