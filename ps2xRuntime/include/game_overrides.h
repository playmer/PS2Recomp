...
#define PS2_REGISTER_GAME_OVERRIDE(NAME_LITERAL, ELF_NAME_LITERAL, ENTRY_U32,  \
                                   CRC32_U32, APPLY_FN)                        \
  namespace {                                                                  \
  static const ps2_game_overrides::Descriptor                                  \
      PS2_GAME_OVERRIDE_JOIN(kPs2OverrideDesc_, __LINE__){                     \
          NAME_LITERAL, ELF_NAME_LITERAL, static_cast<uint32_t>(ENTRY_U32),    \
          static_cast<uint32_t>(CRC32_U32), APPLY_FN};                         \
  static const ps2_game_overrides::AutoRegister PS2_GAME_OVERRIDE_JOIN(        \
      kPs2OverrideReg_,                                                        \
      __LINE__)(PS2_GAME_OVERRIDE_JOIN(kPs2OverrideDesc_, __LINE__));          \
  }
