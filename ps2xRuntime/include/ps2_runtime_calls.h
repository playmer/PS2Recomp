...
inline std::string_view findExact(std::string_view name,
...
inline std::string_view resolveNameWithOptionalLeadingUnderscoreAlias(
    std::string_view name, const std::string_view (&entries)[N]) {
  if (name.empty()) {
    return {};
  }
  ...
  for (auto entry : entries) {
    if (entry.size() == name.size() + 1 && entry.starts_with('_') &&
        entry.substr(1) == name) {
      return entry;
    }
  }
...
inline std::string_view resolveSyscallName(std::string_view name) {
  return detail::resolveNameWithOptionalLeadingUnderscoreAlias(name,
                                                               kSyscallNames);
}
